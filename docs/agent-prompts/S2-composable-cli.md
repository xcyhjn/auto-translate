...
## Context
- 当前 `src/autosub_zh/cli.py` 是单体参数入口，真正的流水线逻辑在 `src/autosub_zh/pipeline_runner.py`/`src/autosub_zh/pipeline_core.py`，UI 直接调用 Python API。
- 现有参数和默认值必须保持；用户已有脚本不能因 parser 重构失效。
- `src/autosub_zh/qa.py`、`src/autosub_zh/pipeline_core.py` 已有可调用函数；先检查真实签名，不要猜测。
...
## Scope
- 只修改：`src/autosub_zh/cli.py`、可新建 `src/autosub_zh/cli_commands.py`、新增 `tests/test_cli_commands.py`。
- 可读取：`src/autosub_zh/pipeline_runner.py`、`src/autosub_zh/pipeline_core.py`、`src/autosub_zh/translate.py`、`src/autosub_zh/qa.py`、`src/autosub_zh/subtitle_io.py`、`src/autosub_zh/workflow_profiles.py`。
- 不得修改：`src/autosub_zh/ui_server.py`、`src/autosub_zh/pipeline_core.py`、`src/autosub_zh/job_store.py`、`src/autosub_zh/worker_service.py`、`requirements.txt`、数据库和生成产物。
...
## Stop Conditions
停止并返回阻塞信息：需要改变 `src/autosub_zh/pipeline_core.py` 签名、需要新增依赖、需要修改 UI/API、需要改输出命名、需要删除旧参数或 scope 外文件。
